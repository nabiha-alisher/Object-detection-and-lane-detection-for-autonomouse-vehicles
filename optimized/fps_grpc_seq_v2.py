...
class TrafficTritonModel:
    ...
    def predict(self, source, imgsz=640, conf=0.25, device=0, verbose=False):
        ...
        t0 = time.perf_counter()
        result = self.client.infer(model_name=self.model_name, inputs=[inp], outputs=[out_req])
...
if __name__ == "__main__":
    ...
    suffix = os.environ.get("FPS_NAME_SUFFIX", "fps_grpc_seq_v2")
    lane_strategy = os.environ.get("FPS_LANE_STRATEGY", "corridor_memory_two_hits")

    print("\n=== FPS VARIANT START ===")
    print("variant:", "grpc_seq")
