...
# Thread-local Triton clients for parallel inference.
# This avoids reusing one HTTP/gRPC client object across Python worker threads.
_triton_client_tls = threading.local()

def _get_thread_triton_client():
    c = getattr(_triton_client_tls, "client", None)
    if c is None:
        c = httpclient.InferenceServerClient(url=TRITON_URL)
        _triton_client_tls.client = c
    return c
...
print("Triton gRPC URL:", TRITON_URL)


def _meta_to_dict(x):
    if isinstance(x, dict):
        return x
    if hasattr(x, "as_json"):
        y = x.as_json()
        if isinstance(y, str):
            return json.loads(y)
        return y
    if hasattr(x, "inputs") and hasattr(x, "outputs"):
        def _one(t):
            return {
                "name": getattr(t, "name"),
                "shape": [int(v) for v in getattr(t, "shape")],
                "datatype": getattr(t, "datatype"),
            }
        return {
            "inputs": [_one(t) for t in x.inputs],
            "outputs": [_one(t) for t in x.outputs],
        }
    raise TypeError(f"Unsupported Triton metadata object: {type(x)!r}")
...
class TritonTRTRunner:
    def __init__(self, model_name):
        ...
        assert self.client.is_model_ready(model_name), f"Triton model not ready: {model_name}"

        meta = _meta_to_dict(self.client.get_model_metadata(model_name=model_name))
    ...
    def forward(self, x):
        ...
        result = _get_thread_triton_client().infer(
...
class TrafficTritonModel:
    ...
    def predict(self, source, imgsz=640, conf=0.25, device=0, verbose=False):
        ...
        t0 = time.perf_counter()
        result = _get_thread_triton_client().infer(model_name=self.model_name, inputs=[inp], outputs=[out_req])
...
# Persistent inference pool for parallel Triton model calls
_infer_pool = ThreadPoolExecutor(max_workers=3)
...
def render_one_frame(frame_bgr, src_w, src_h, lane_strategy=DEFAULT_LANE_STRATEGY, lane_memory=None):
    ...
    # Parallel Triton model calls:
    # YOLOPX + Depth + Traffic are independent for this frame.
    def _traffic_predict_call():
        t0_local = time.perf_counter()
        res_local = traffic_model.predict(
            source=frame_bgr, imgsz=TRAFFIC_IMGSZ,
            conf=TRAFFIC_CONF_THRES, device=0, verbose=False
        )[0]
        return res_local, (time.perf_counter() - t0_local)

    t_model_group0 = time.perf_counter()

    f_yolopx_inf = _infer_pool.submit(yolopx_runner.forward, yolopx_x)
    f_depth_inf = _infer_pool.submit(depth_runner.forward, depth_x)
    f_traffic_inf = _infer_pool.submit(_traffic_predict_call)

    yolopx_outs, yolopx_ms = f_yolopx_inf.result()
    depth_outs, depth_ms = f_depth_inf.result()
    traffic_res, traffic_total_s = f_traffic_inf.result()

    timings["model_group_wall"] += time.perf_counter() - t_model_group0

    timings["yolopx_forward_s"] += yolopx_ms / 1000.0
    timings["depth_forward_s"] += depth_ms / 1000.0
    timings["traffic_total_call"] += traffic_total_s
...
if __name__ == "__main__":
    ...
    suffix = os.environ.get("FPS_NAME_SUFFIX", "fps_grpc_parallel_v2")
    lane_strategy = os.environ.get("FPS_LANE_STRATEGY", "corridor_memory_two_hits")

    print("\n=== FPS VARIANT START ===")
    print("variant:", "grpc_parallel")
