...
# ──────────────────────────────────────────────────────────────
TRITON_URL = None  # RAW_LOCAL_TRT_PATCH
triton_client = None  # RAW_LOCAL_TRT_PATCH
# RAW_LOCAL_TRT_PATCH: Triton assert removed
# RAW_LOCAL_TRT_PATCH: Triton assert removed
print("Inference backend: RAW_LOCAL_TENSORRT")
...
class TrafficRawTensorRTModel(TrafficTritonModel):
    """
    Apple-to-apple traffic backend:
    - inherits the exact same _letterbox(), _preprocess(), and _postprocess()
      from TrafficTritonModel
    - replaces only Triton HTTP inference with local TensorRT execute_async_v3()
    """

    def __init__(self, engine_path=TRAFFIC_ENGINE):
        self.model_name = "traffic_raw_local_trt"
        ...
        if self.engine is None:
            raise RuntimeError(f"Failed to deserialize traffic engine: {engine_path}")

        self.context = self.engine.create_execution_context()
        if self.context is None:
            raise RuntimeError(f"Failed to create traffic execution context: {engine_path}")
        ...
        if self.input_name is None:
            raise RuntimeError("Traffic engine input tensor not found")
        if not self.output_names:
            raise RuntimeError("Traffic engine output tensor not found")
        ...
        self.input_torch_dtype = self._trt_dtype_to_torch(self.input_trt_dtype)
        self.input_np_dtype = self._trt_dtype_to_numpy(self.input_trt_dtype)
        ...
        for n in self.output_names:
            ...
            dt = self._trt_dtype_to_torch(self.engine.get_tensor_dtype(n))
        ...
        print("Traffic class names:", self.names)
        print("TrafficRawTensorRTModel ready")
        print(" traffic input :", self.input_name, self.input_shape, self.input_torch_dtype)
        print(" traffic output:", self.output_name, tuple(self.context.get_tensor_shape(self.output_name)))
    ...
    def predict(self, source, imgsz=640, conf=0.25, device=0, verbose=False):
        ...
        if tuple(arr.shape) != tuple(self.input_shape):
            raise RuntimeError(f"Traffic input shape mismatch: arr={arr.shape}, engine={self.input_shape}")

        x = torch.from_numpy(arr).to(globals()["device"], non_blocking=True)

        if self.input_buf.dtype == torch.float16:
            self.input_buf.copy_(x.half())
        elif self.input_buf.dtype == torch.float32:
        ...
        if not ok:
            raise RuntimeError("TrafficRawTensorRTModel execute_async_v3 failed")
        self.e1.record()

        torch.cuda.synchronize()
        inf_ms = self.e0.elapsed_time(self.e1)

        raw = self.output_tensors[self.output_name].detach().cpu().numpy()
        boxes = self._postprocess(raw, ratio, pad, source.shape, conf)

        return [_TrafficResult(boxes=boxes, speed={"inference": float(inf_ms)})]


traffic_model = TrafficRawTensorRTModel(TRAFFIC_ENGINE)
...
def _find_peaks_strict(hist, thr, min_sep):
    # VECTORIZED - see verify_find_peaks.py (20,000-case equivalence proof)
    n = len(hist)
    if n < 3:
        return []
    h = np.asarray(hist)
...
if __name__ == "__main__":
    minutes = float(os.environ.get("FPS_TEST_MINUTES", "1.0"))
    suffix = os.environ.get("FPS_NAME_SUFFIX", "fps_rawtrt_seq_v2")
    lane_strategy = os.environ.get("FPS_LANE_STRATEGY", "corridor_memory_two_hits")

    print("\n=== FPS VARIANT START ===")
    print("variant:", "rawtrt_seq")
    print("minutes:", minutes)
    print("suffix:", suffix)
    print("lane_strategy:", lane_strategy)
    ...
    try:
        ...
        print("=== FPS VARIANT DONE ===")
        print(json.dumps({
            "variant": "rawtrt_seq_v2",
            "summary_json": result.get("csv_output", "").replace("_frames.csv", "_summary.json"),
            "end_to_end_fps": result.get("end_to_end_fps"),
            "end_to_end_ms_per_frame": result.get("end_to_end_ms_per_frame"),
            "combined_forward_ms_per_frame": result.get("combined_forward_ms_per_frame"),
            "lane_post_and_decision_ms_per_frame": result.get("lane_post_and_decision_ms_per_frame"),
            "traffic_total_call_ms_per_frame": result.get("traffic_total_call_ms_per_frame"),
            "write_ms_per_frame": result.get("write_ms_per_frame"),
        }, indent=2))
    finally:
        if "_ctx" in globals() and _ctx is not None:
            try:
